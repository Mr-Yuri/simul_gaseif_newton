...
def biomass_M_mixt(ydaf_dict): #massa molecular da mistura / recalcular M_bio com as composições normalizadas ~24 kg/kmol
...
def simulation(ER, humidity, SB=0.0, Tg=1113.0, Tair=600, mdot_bio=1.0, log_file=None, no_print=False):
    ...
    def stq_model_function(x):
        ...
        f[3] = calculated_Kp["Kp1"]*(n2*n4) - (n1*n3)
        f[4] = calculated_Kp["Kp5"]*(n1**2) - (n5)*n_t
    ...
    n_H2O = n_p[0] + 2.0*n_p[4] + n_p[-1]           # n_H2O = n1 + 2*n5 + n9
    n_SO2 = n_p[-1]                                          # n_SO2 = n9
    ...
    print_message(f"HHV do gás produzido em base umida: HHV_g = {round(HHV_g, 3)} kJ/kmol\n",log_file=log_file, no_print=no_print)
    print_message(f"LHV do gás produzido em base umida: LHV_g = {round(LHV_g_vol, 3)} MJ/Nm3 ou LHV_g = {round(LHV_g, 3)} kJ/kmol\n",log_file=log_file, no_print=no_print)
    print_message(f"LHV do gás produzido: LHV_g = {round(LHV_g/M_g_mixt, 3)} kJ/kg\n",log_file=log_file, no_print=no_print)
    ...
    # ex_ch_reac_total = sum([x_r[i]*ex_ch_reac_default[key] for i,key in enumerate(ex_ch_reac_default.keys())]) + x_log_sum(x_r)
    ...
    # Gás produzido
    rho_air = 1.169
    Vair = ((AC_real1)*mdot_bio*(1-humidity/100-biomass_inputs["cinzas"]/100))/rho_air
    Vgp = Vair*0.79/x_pd[-2]
    M_prods = [M_H2, M_CO, M_CO2, M_CH4, M_N2, M_H2S]
    M_gp = sum([x_pd[i]*M_prods[i] for i in range(len(x_pd))])
    rho_gp = P*(M_gp/1000)/(Ru*Tref)
    mdot_gp = Vgp*rho_gp
    LHV_gp = LHV_g/M_gp
    LHV_bio = LHV_d_bio/(1-humidity/100)

    cge = 100*(LHV_gp)/(LHV_bio)
    ...
    print_message(f"Eficiência de gás frio (Loha 2011): n_cg = {round(cge, 3)}%\n",log_file=log_file, no_print=no_print)
    ...
    return x, statistics(x), LHV_g_vol

xx = np.linspace(0.2, 0.5, 21)
yy = np.linspace(0, 20, 21)
def create_graph_data1(log_file=None, no_print=True):
    result_dict = {}
    for er in xx:
        mc_dict = {}
        for mc in yy:
            x, result, lhv = simulation(er, mc, log_file=log_file, no_print=no_print)
            result["LHV"] = lhv
            mc_dict[str(mc)] = result
        result_dict[str(er)] = mc_dict
    return result_dict

def create_graph_dataT(log_file=None, no_print=True):
    result_list=[]
    tg = np.linspace(600, 950, 8)
    sb = np.array([0.,0.3,0.6])
    for s in sb:
        partial_result=[]
        for t in tg:
            x, stats, lhv = simulation(0.3, 8.0, SB=s, Tg=t+273.15, log_file=log_file, no_print=no_print)
            stats["LHV"] = lhv
            partial_result.append(stats)
        result_list.append(partial_result)
    return tg, result_list

def elem_graph1(elem_name, result_dict, log_file=None, no_print=False):
    print_message(f"Gerando gráfico para {elem_name}.\nVariando ER entre {xx.min()} e {xx.max()}\n", log_file=log_file, no_print=no_print)
    print_message(f"Variando Umidade entre {yy.min()}% e {yy.max()}%\n", log_file=log_file, no_print=no_print)
    def elem_func(x, y):
    ...
    if elem_name == "LHV":
        ...
        comp = "%"
    print_message(f"Valor máximo de {elem_name} ({round(k, 3)} {comp}) encontrado em: ER = {xx[i[0]]} e Umidade = {yy[j[0]]}%\n", log_file=log_file, no_print=no_print)
    fig, ax = plt.subplots()
    cb_ticks = np.linspace(round(zz.min(), 2), round(zz.max(), 2), 7)
    cax = ax.contourf(xx, yy, zz, levels=500, cmap="jet")
    ax.set_xticks([0.2, 0.25, 0.30, 0.35, 0.40, 0.45, 0.5])
    ax.set_yticks([i for i in range(21) if i%4==0])
    ax.set_xlabel("ER")
    ax.set_ylabel("MC [%]")
    if elem_name == "LHV":
        ax.set_title(f"{elem_name} [MJ/Nm³]")
    else:
        ax.set_title(f"{elem_name} [%]")
    
    cbar = fig.colorbar(cax, ticks=list(cb_ticks))
    plt.savefig(f"./img/{elem_name}_final.png")
    plt.clf()
    plt.close()
    return True

def elem_graphT(elem_name, tg, result_list, log_file=None, no_print=False):
    print_message(f"Gerando gráfico para {elem_name}.\nVariando Tg entre {tg.min()}K e {tg.max()}K\nVariando SB entre 0.0 e 0.6\n", log_file=log_file, no_print=no_print)
    if elem_name == "LHV":
        comp = "MJ/Nm³"
    else:
        comp = "%"
    data1, data2, data3 = result_list
    xelem1= [e[elem_name] for e in data1]
    xelem2= [e[elem_name] for e in data2]
    xelem3= [e[elem_name] for e in data3]

    fig, ax = plt.subplots()
    ax.plot(tg, xelem1, marker="o", color="black", label="SB = 0.0")
    ax.plot(tg, xelem2, marker="v", color="red", label="SB = 0.3")
    ax.plot(tg, xelem3, marker="s", color="dimgrey", label="SB = 0.6")

    ax.set(xlabel='Temperatura do gaseificador, [°C]', ylabel=f"{elem_name} [{comp}]")
    ax.legend()
    ax.grid()

    plt.savefig(f"./img2d/{elem_name}_T_SB.png")
    plt.clf()
    plt.close()
    return True

# tg, data = create_graph_dataT()
# elem_graphT("H2", tg, data)
# elem_graphT("LHV", tg, data)
# elem_graphT("H2", tg, data)
# elem_graphT("H2", tg, data)
# elem_graphT("CO", tg, data)
# elem_graphT("CO2", tg, data)
# elem_graphT("CH4", tg, data)
# elem_graphT("N2", tg, data)
# x, stats, lhv = simulation(ER=0.32, SB=0.0, humidity=16.0,Tg=1113.0, Tair=600.0, mdot_bio=1.0, no_print=False)

# simulation(ER=0.32, SB=0.0, humidity=16.0,Tg=1100, mdot_bio=40.0)
FILE_NAME = "main_colormap"
now = str(datetime.datetime.now().strftime("%Y-%m-%d"))
with open(f"./logs/{FILE_NAME.replace('.py', '')}_{now}.txt", "w+", encoding="utf-8") as file:
    # x, stats, lhv = simulation(ER=0.32, SB=0.0, humidity=16.0,Tg=1113.0, Tair=600.0, mdot_bio=1.0, log_file=file)
# with open(f"./logs/final/{FILE_NAME.replace('.py', '')}_case1.txt", "w+", encoding="utf-8") as file:
#     x, stats, lhv = simulation(ER=0.25, SB=0.4, humidity=12.0,Tg=1100.0, Tair=600.0, mdot_bio=1.0, log_file=file, no_print=True)
# with open(f"./logs/final/{FILE_NAME.replace('.py', '')}_case2.txt", "w+", encoding="utf-8") as file:
#     x, stats, lhv = simulation(ER=0.25, SB=0.8, humidity=12.0,Tg=1100.0, Tair=600.0, mdot_bio=1.0, log_file=file, no_print=True)
# with open(f"./logs/final/{FILE_NAME.replace('.py', '')}_case3.txt", "w+", encoding="utf-8") as file:
#     x, stats, lhv = simulation(ER=0.4, SB=0.8, humidity=12.0,Tg=1100.0, Tair=600.0, mdot_bio=1.0, log_file=file, no_print=True)
    data = create_graph_data1(log_file=file)
    # elem_graph1("LHV", data, log_file=file)
    # elem_graph1("H2", data, log_file=file)
    # elem_graph1("CO", data, log_file=file)
    # elem_graph1("CO2", data, log_file=file)
    elem_graph1("CH4", data, log_file=file)
    # elem_graph1("N2", data, log_file=file)