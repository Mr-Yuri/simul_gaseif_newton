...
def simulation(ER, humidity, SB=0.0, Tg=1113.0, Tair=600, mdot_bio=1.0, log_file=None, no_print=False):
    ...
    # Defining the corrections alfa(T) and beta to modify the simple equilibrium equations (M2) according to Mendiburu (2014)
    def kp_alfa(T):
        eq = -1.639*(1e-4)*T**2 + 0.3518*T - 128.7
        out = max(eq, 1)
        return out
    kp_beta = 2.8 - 0.372*3.76
    ...
    def stq_model_function(x):
        ...
        f[3] = kp_beta*calculated_Kp["Kp1"]*(n2*n4) - (n1*n3)
        f[4] = kp_alfa(Tg)*calculated_Kp["Kp5"]*(n1**2) - (n5)*n_t
    ...
    print_message(f"LHV do gás produzido em base umida: LHV_g = {round(LHV_g_vol, 3)} MJ/Nm³\n",log_file=log_file, no_print=no_print)
    ...
    def statistics(x):
        # print(f"Resultado Newton-Raphson: {x}")
        nt = x[-1]
        n_H2O = x[3]
        x = 100*x/(nt - n_H2O)
